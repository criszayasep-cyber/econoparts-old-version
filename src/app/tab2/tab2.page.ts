...
export class Tab2Page implements OnInit {
  ...
  public SizeOptionsEquivalentes: number[] = [];
  public SizeOptionsIntercambios: number[] = [];
  ...
  async loadCombos() {
    if (this.pf.isBrowser()) {
      ...
      if (http.ok) {
        ...
        console.log("grupos:", this.combos);
        this.buscar(true)
        //this.combos.subgrupos = http.registros;
  ...
  productosFiltrados() {
    ...
    return this.productos.filter(item =>
      item.pro_number?.toLowerCase().includes(texto) ||
      item.pro_descripcion?.toLowerCase().includes(texto)
    );
  }
  ...
  async buscar(primera: boolean) {
    ...
    if (this.pf.isBrowser()) {
      ...
      if (http.ok) {
        ...
        this.total = http.total;
        if (this.total > 1000) {
          const limiteSuperior = Math.ceil(this.total / 1000) * 1000;
          for (let i = 1000; i <= limiteSuperior; i += 1000) {
    ...
    if (this.configuracion.ConfiguracionService.online) {
      ...
      if (response.ok) {
        ...
        if (producto.pro_number != "510102") {
          ...
          if (http) {
            if (http.ok) {
              if (http.mensaje.length > 0)
                this.tools.showNotificationWithInput("Exito", "Producto agregado exitosamente" + http.mensaje, "Ok", http.token, this.pedidoService, dataPost);
              else
  ...
  limpiarIntercambios() {
    this.registros.intercambios = -1;
    this.productosIntercambios = [];
    this.codigo = "";
  }
  ...
  async buscarEquivalentes() {
    ...
    if (this.pf.isBrowser()) {
      ...
      if (http.ok) {
        this.productosEquivalentes = http.registros;
        console.log(http)
        this.totalEquivalentes = http.total;
        if (this.totalEquivalentes > 1000) {
          const limiteSuperior = Math.ceil(this.totalEquivalentes / 1000) * 1000;
          for (let i = 1000; i <= limiteSuperior; i += 1000) {
            this.SizeOptionsEquivalentes.push(i);
          }
        } else {
          const limiteSuperior = Math.ceil(this.totalEquivalentes / 5) * 5;
          for (let i = 5; i <= limiteSuperior; i += 5) {
            this.SizeOptionsEquivalentes.push(i);
          }
        this.registros.equivalentes = this.productosEquivalentes.length;
  ...
  //#region BUSQUEDA INTERCAMBIOS
  async buscarIntercambios() {
    this.registros.intercambios = -1;
    this.productosIntercambios = [];
    this.loading.intercambios = true;
    ...
    if (this.pf.isBrowser()) {
      ...
      var http = await this.productoService.intercambios(dataPost);
      if (http.ok) {
        this.productosIntercambios = http.registros;
        this.registros.intercambios = this.productosIntercambios.length;

        console.log("intercambios:", this.productosIntercambios);
        this.totalIntercambios = http.total;
        if (this.totalIntercambios > 1000) {
          const limiteSuperior = Math.ceil(this.totalIntercambios / 1000) * 1000;
          for (let i = 1000; i <= limiteSuperior; i += 1000) {
            this.SizeOptionsIntercambios.push(i);
          }
        } else {
          const limiteSuperior = Math.ceil(this.totalIntercambios / 5) * 5;
          for (let i = 5; i <= limiteSuperior; i += 5) {
            this.SizeOptionsIntercambios.push(i);
          }
        }
      ...
      this.loading.intercambios = false;
    } else {
      var query = `SELECT * FROM venta_movil_productos 
                    INNER JOIN venta_movil_inventario_precios ON no=pro_number
                    WHERE bodega='${this.bodegaEq}' AND pro_number IN (SELECT noSustituto FROM venta_movil_equivalentes WHERE no='${this.codigo}')
                    ORDER BY existencia DESC `
      var prds = await this.db.select(query);
      this.productosIntercambios = prds;
      this.registros.intercambios = prds.length;

      this.loading.intercambios = false;

      if (this.registros.intercambios > 0 && this.configuracion.ConfiguracionService.online) {
        //Enviamos los codigos para búsqueda
        var cods = "'" + prds.map(a => a.pro_number).join("','") + "'"
        var dataPost = {
          busqueda: cods,
        ...
        if (http.ok) {
          ...
          var jsonUpdate = [];
          actualizacion.forEach(element => {
            var index = this.productosIntercambios.findIndex(obj => obj.pro_number == element.no)
            this.productosIntercambios[index].preciou = element.preciou;
            this.productosIntercambios[index].existencia = element.existencia;
            //Actualizar la DB
            jsonUpdate.push({
              "set": {
                "preciou": element.preciou,
